    ...
    <div class="service-wrapper">
        <div class="form-container">
            ...
            <h4 class="section-title"><i class="fa fa-user text-primary"></i> 1. Datos del Cliente</h4>
             <div class="row">
                <div class="col-md-8">
                    <?= $form->field($model, 'cliente_nombre')->widget(Select2::class, [
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                        <label class="control-label">Hora del Servicio</label>
                        <?= Html::input('time', 'CotizacionRapida[hora_servicio]', '', [
                            'class' => 'form-control campo-hora-dinamico', // Clase CRÍTICA para el recargo
                            'style' => 'border-radius: 10px;',
                            'id' => 'campo-hora'
                        ]) ?>
                </div>
                <div class="col-md-2">
                    <label>Ruta (Servicio)</label>
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <?= $form->field($model, 'monto_viatico')->textInput(['id' => 'monto-viatico', 'class' => 'form-control calc-trigger']) ?>
                </div>
            </div>
        ...
        <div class="summary-sticky">
            <div class="card-total">
                ...
                <?= $form->field($model, 'monto_total')->hiddenInput(['id' => 'final-monto-input'])->label(false) ?>
...
$this->registerJs("
    ...
    if(!val) return 0;
    // Quita los puntos de miles y cambia la coma decimal por punto para que JS lo entienda
   ...
   function updateCalculo() {
    ...
    let recargo = getFloat($('#monto-recargo').val()); // Aquí lee el recargo del AJAX
    ...
    setFlag('loading', 'CONSULTANDO HORARIO...');
    
    $.get('<?= $urlRecargo ?>', { hora: hora }, function(res) {
        if (res.success) {
            var valor = parseFloat(res.recargo) || 0;
            // Formateamos con coma para mantener consistencia
            $('#monto-recargo').val(valor.toLocaleString('de-DE', {minimumFractionDigits: 2}));
            
            setFlag('success', valor > 0 ? 'RECARGO APLICADO: ' + res.descripcion : 'TARIFA NORMAL');
            updateCalculo(); // Recalcula el total general
        }
    });
}

// Disparar cuando la hora cambie
$(document).on('change blur', '.campo-hora-dinamico', function() { 
    obtenerRecargo(); 
});
");
?>